...
select = Listbox(frame, yscrollcommand=scroll.set, height=20, width=40)
...
def add_contact():
    contact = dict(name=Name.get(), surname=Surname.get(), phone=Phone.get(), address=Address.get(), email=Email.get())
...
def delete():
    del my_dict[selected()]
    select_set()


def view():
    selected_contact = selected()
    name = my_dict[selected_contact]['name']
    surname = my_dict[selected_contact]['surname']

    # name, surname, phone, address, email = my_dict[selected()]
